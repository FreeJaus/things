...
for (i = [0:4]) {
  translate([i*(2*base_radius+clearance-4),0,0]) settlement();
}

translate([0,(2*base_radius+clearance),0]) for (i = [0:3]) {
  translate([i*(2*base_radius+clearance-4),0,0]) {
    city();
  }
}
...
for (j = [0:1]) {
  for (i = [0:2]) {
    translate([i*(2*base_radius+clearance-4),j*(2*base_radius+.75*clearance),0]) knight(i+1);
  }
}
...
for (j = [0:2]) {
  for (i = [0:1]) {
    translate([i*5, i*6.5+j*13,5+2*nozzle]) rotate([180,0,0]) knight_flag();
  }
}
